...
async def get_b_user_id( id: str , db: Session = Depends(get_db)):
    return await service.get_b_user_id(db , id)

@router.post('/b_user/')
async def post_b_user( id: int, user_name: str , db: Session = Depends(get_db)):
    return await service.post_b_user(db , id, user_name)
...
@router.post('/novostackcrm')
async def post_novostackcrm( id: int, name: str , db: Session = Depends(get_db)):
    return await service.post_novostackcrm(db , id, name)
