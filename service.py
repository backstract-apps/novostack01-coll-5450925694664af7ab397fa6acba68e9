...
async def get_b_user_id(db: Session, id: str):
...
async def post_b_user(db: Session, id: int, user_name: str):
...
async def post_novostackcrm(db: Session, id: int, name: str):


    user_record = db.query(models.B_user).filter(models.B_user.id == id).first()
    res = {
        'hjghjjh': name,
    }
    return res
